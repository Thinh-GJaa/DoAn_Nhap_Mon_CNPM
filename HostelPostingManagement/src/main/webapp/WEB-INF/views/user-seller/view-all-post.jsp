	...
	<div class="container">
		...
		<div class="border rounded">
			<div style="background-color: rgb(237, 240, 243);">
				<h6 class="pt-3 ml-3 ">Danh sách tất cả bài đăng</h6>
				<hr>
			</div>
			<div class="container">
					...
					<tbody>
						<c:forEach var="post" items="${listPosts}">
							...
							<tr>
								...
								<td><c:choose>
										<c:when test="${post.isApprove() == true}">Có</c:when>
										<c:otherwise>Chưa</c:otherwise>
									</c:choose></td>
								<td><c:choose>
										<c:when test="${post.isHidden() == true}">Có</c:when>
								...
								<td class="d-flex"><div>
										<a class="btn btn-outline-secondary mr-1 btn-sm"
											...
											href="${contextPath}/${detailLink}"><i class="bi bi-info-square "></i></a>
									</div>
									<div>
										<a class="btn btn-outline-secondary mr-1 btn-sm"
											data-toggle="tooltip" data-placement="top"
					...
					<div class="text-right">
						...
						<c:forEach begin="1" end="${totalPages}" var="i">
							<c:choose>
								<c:when test="${currentPage != i}">
									<a href="${contextPath}/list-user-post/page/${i}">[${i}]</a>&nbsp;
								</c:when>
								<c:otherwise>
									<span>[${i}]</span>&nbsp;
								</c:otherwise>
						...
						<c:choose>
							<c:when test="${currentPage < totalPages}">
								<a href="${contextPath}/list-user-post/page/${currentPage+1}"><i
						...
						<c:choose>
							<c:when test="${currentPage < totalPages}">
								<a href="${contextPath}/list-user-post/page/${totalPages}">Last</a>&nbsp;
							</c:when>
						</c:choose>
					</div>
				</c:if>
