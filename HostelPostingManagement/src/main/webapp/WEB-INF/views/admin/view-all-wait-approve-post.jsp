				...
				<table class="table table-bordered table table-striped">
					...
					<tbody>
						<c:forEach var="post" items="${listPosts}">
							<c:url var="approveLink" value="approve-post">
								<c:param name="postId" value="${post.id}"></c:param>
							</c:url>
							<c:url var="notApproveLink" value="hidden-post">
								<c:param name="postId" value="${post.id}"></c:param>
							</c:url>
							...
							<tr>
								...
								<td class="d-flex"><a
									...
									data-toggle="tooltip" data-placement="top" title="Duyệt"
									href="${contextPath}/${approveLink}"><i
										class="bi bi-check text-success"></i></a> <a
									class="btn btn-outline-secondary mr-1 btn-sm" 
									data-toggle="tooltip" data-placement="top" title="Không duyệt"
									href="${contextPath}/${notApproveLink}"><i
										class="bi bi-x text-danger"></i></a> <a
									class="btn btn-outline-secondary mr-1 btn-sm"
					...
					<div class="text-right">
						...
						<c:forEach begin="1" end="${totalPages}" var="i">
							<c:choose>
								<c:when test="${currentPage != i}">
									<a href="${contextPath}/all-list-post-wait-approve/page/${i}">[${i}]</a>&nbsp;
								</c:when>
								<c:otherwise>
									<span>[${i}]</span>&nbsp;
								</c:otherwise>
						...
						<c:choose>
							<c:when test="${currentPage < totalPages}">
								<a
									href="${contextPath}/all-list-post-wait-approve/page/${currentPage+1}"><i
						...
						<c:choose>
							<c:when test="${currentPage < totalPages}">
								<a
									href="${contextPath}/all-list-post-wait-approve/page/${totalPages}">Last</a>&nbsp;
							</c:when>
						</c:choose>
					</div>
				</c:if>
